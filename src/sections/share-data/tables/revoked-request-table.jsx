...
export default function RevokedRequestTableView({
  filterName,
  selected,
  setSelected,
  revokedDataRequest,
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = revokedDataRequest.map((n) => n.name);
  ...
  return (
    <>
        ...
        <TableContainer sx={{ overflow: 'unset' }}>
          <Table sx={{ minWidth: 800 }}>
            <CommonTableHead
              order={order}
              orderBy={orderBy}
              rowCount={revokedDataRequest.length}
            ...
            <TableBody>
              ...
              <TableEmptyRows
                height={77}
                emptyRows={emptyRows(page, rowsPerPage, revokedDataRequest.length)}
              />
      ...
      {revokedDataRequest.length > rowsPerPage && (
        <TablePagination
          page={page}
          component="div"
          count={revokedDataRequest.length}
...
RevokedRequestTableView.propTypes = {
  filterName: PropTypes.string,
  selected: PropTypes.array,
  setSelected: PropTypes.func,
  approveRequest: PropTypes.func,
  revokedDataRequest: PropTypes.array,
};
